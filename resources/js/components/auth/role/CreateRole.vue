	 ...
	 <div v-if="showing" class="fixed inset-0 w-full h-screen flex items-center justify-center bg-semi-75">
		<div class="w-full max-w-2xl bg-white shadow-lg rounded-lg p-4">
			...
			<div class="text-right flex justify-between items-center">
				<h1 class="font-semibold text-2xl text-gray-700">Create New Role</h1>
			...
			<div class="mt-8">
				<label class="block">
					<button class="bg-gray-700  py-1 px-2 h-10 w-32 rounded text-white hover:bg-gray-500" @click="sendRole()">Save</button>
				</label>
			</div> 
		</div>
	</div>
	...
	export default{
         ...
         methods:{
         	...
         	sendRole(){
         		...
         		axios.post('/roles/store',this.form).then((res) => {
                  if(res){
